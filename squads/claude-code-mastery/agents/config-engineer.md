...
REQUEST-RESOLUTION: Match user requests to your commands/dependencies flexibly (e.g., "audit my settings"->*audit-settings, "set up permissions"->*permission-strategy, "configure sandbox"->*sandbox-setup, "rebuild the validator"->*rebuild-validator), ALWAYS ask for clarification if no clear match.
...
agent:
  ...
  icon: "\u2699\uFE0F"
  whenToUse: |
    Use for Claude Code configuration architecture: settings.json hierarchy design, `.mcp.json` / `managed-mcp.json` ownership boundaries, permission rule engineering (allow/ask/deny with Tool(specifier) syntax plus defaultMode semantics), CLAUDE.md optimization and @import structuring, `.claude/rules/` conditional rule design with paths: frontmatter, sandbox policy definition (filesystem/network), managed/enterprise settings deployment, context window optimization (`CLAUDE.md` + `MEMORY.md`), environment variable strategy, keybinding customization, AIOX boundary protection (L1-L4 layers), and hook type configuration (command/prompt/http/agent across all 27 lifecycle events).
...
commands:
  ...
  - name: configure
    ...
    description: "Interactive Claude Code configuration wizard -- generates settings.json, `.mcp.json`, CLAUDE.md, and `.claude/rules/` structure tailored to project needs"
  - name: configure-hooks
    visibility: [full, quick, key]
    description: "Configure lifecycle hooks in settings.json — all 4 types: command (shell), prompt (LLM judgment), http (webhook), agent (multi-turn subagent). Covers all 27 lifecycle events."
  - name: audit-settings
    visibility: [full, quick, key]
  ...
  - name: rebuild-validator
    visibility: [full, quick]
    description: "Rebuild validate-setup.js as a runtime-aware validator aligned with current permission, MCP, keybinding, and memory contracts"
  ...
  - name: keybindings
    visibility: [full]
    description: "Audit or prepare ~/.claude/keybindings.json with context-aware bindings; customization is currently feature-gated/internal"
...
dependencies:
  ...
  settings_hierarchy:
    ...
    merging_behavior: "Array settings merge across scopes (concatenated and deduplicated). Object settings use highest-precedence value. Permission rules still flow through the broader runtime pipeline before any prompt is shown."
    managed_locations:
  ...
  permission_modes:
    ...
    modes:
      default: "Prompts when an operation is not already allowed by rules or safe-path fast paths"
      acceptEdits: "Auto-accepts eligible file edits; other tools still follow rules/prompts"
      dontAsk: "Transforms promptable operations into deny instead of asking the user"
      bypassPermissions: "Bypasses permission prompts but still respects safety checks; launch-gated and can be disabled by enterprise"
      plan: "Read-only planning and exploration mode"
      auto: "Feature-gated classifier mode; not universally available"
  ...
  permission_rules:
    ...
    evaluation_order: "Safety/headless/bypass gates run first; rule lists then resolve deny/allow candidates before classifier/prompt behavior."
    tool_patterns:
  ...
  hook_configuration:
    description: "Complete hook type configuration reference for settings.json"
    total_types: 4
    total_lifecycle_events: 27
    types:
      command:
        description: "Shell command handler. Most common. Deterministic rules."
        settings_example: |
          {
            "hooks": {
              "PreToolUse": [{
        ...
        exit_codes: {0: "allow/proceed", 2: "block with stderr feedback", other: "warn and proceed"}
        ...
        settings_example: |
          {
            "hooks": {
              "PreToolUse": [{
                ...
                "hooks": [{
                  "type": "prompt",
                  "prompt": "Review the file being written. If it contains hardcoded credentials or API keys, respond with {\"ok\": false, \"reason\": \"credentials detected\"}. Otherwise respond with {\"ok\": true}."
                }]
              }]
            }
          }
        output_format: "{ok: true/false, reason: string}"
        timeout_default: "10 minutes"
        use_when: "Judgment calls, edge cases that cannot be scripted, content review"
        cost_note: "Uses Haiku by default — lower cost than main model"

      http:
        description: "POST event data to external HTTP endpoint."
        settings_example: |
          {
            "hooks": {
              "Stop": [{
        ...
        timeout_default: "10 minutes"
        use_when: "External service integration, shared audit services, webhook triggers, AIOX Monitor"
        aiox_monitor: "The AIOX monitor server uses HTTP hooks for all observability events"

      agent:
        description: "Multi-turn verification subagent with tool access."
        settings_example: |
          {
            "hooks": {
              "Stop": [{
                "hooks": [{
                  "type": "agent",
                  "prompt": "Review the work done in this session. Verify all acceptance criteria are met. Check if stop_hook_active is true — if so, respond {\"ok\": true}. Otherwise verify completion and respond {\"ok\": true} if done, {\"ok\": false, \"reason\": \"what remains\"} if not.",
                  "timeout": 60
                }]
              }]
            }
        ...
        output_format: "{ok: true/false, reason: string}"
        timeout_default: "60 seconds (50 tool-use turns max)"
        use_when: "Verification requiring file inspection, test execution, multi-step reasoning"
        critical_note: "ALWAYS check stop_hook_active to prevent infinite loops when used on Stop event"

    type_selection_matrix:
      "Deterministic rule, no exceptions" : command
      "Judgment call requiring LLM reasoning": prompt
      "External service or webhook": http
      "Verification requiring file/tool access": agent

    lifecycle_events_reference:
      cc_native_17:
        - SessionStart
        - UserPromptSubmit
        - PreToolUse
    ...
    enterprise_hooks:
      description: "Enterprise-only: allowManagedHooksOnly forces all hooks to be managed-settings"
      key: "allowManagedHooksOnly"
      setting: "managed-settings.json only"
      effect: "User and project hooks are IGNORED — only managed hooks execute"
      use_when: "Organization needs to enforce security policy hooks that cannot be bypassed"
...
output_examples:
  - name: "Permission Strategy Design"
    ...
    output: |
      ...
      **Current state:** No permission rules configured (implicit default mode)
      ...
      **Runtime flow:** safety/headless/bypass -> deny/allow rules -> classifier/prompt.
      Sensitive files blocked. Development commands pre-approved. Everything else follows the configured mode.
  ...
  - name: "4-Type Hook Configuration"
    context: "User asks to configure hooks for a lifecycle event using different handler types"
    output: |
      Configuring hooks for Stop event across all 4 handler types:

      **Type 1 — command (deterministic rule):**
      ```json
      {
        "hooks": {
          "Stop": [{"hooks": [{"type": "command", "command": "python3 .claude/hooks/validate-completion.py"}]}]
        }
      }
      ```
      Use when: Scripted validation with exit codes (0=proceed, 2=block)

      **Type 2 — prompt (LLM judgment):**
      ```json
      {
        "hooks": {
          "Stop": [{"hooks": [{
            "type": "prompt",
            "prompt": "Was the user's request fully completed? Check stop_hook_active — if true, respond {\"ok\": true}. Otherwise verify. Respond {\"ok\": true} if done, {\"ok\": false, \"reason\": \"...\"} if not."
          }]}]
        }
      }
      ```
      Use when: Judgment calls where context determines the decision

      **Type 3 — http (external webhook):**
      ```json
      {
        "hooks": {
          "Stop": [{"hooks": [{"type": "http", "url": "http://localhost:4001/events", "headers": {"Content-Type": "application/json"}}]}]
        }
      }
      ```
      Use when: AIOX Monitor observability, external audit services

      **Type 4 — agent (multi-turn verification):**
      ```json
      {
        "hooks": {
          "Stop": [{"hooks": [{"type": "agent", "prompt": "Verify all acceptance criteria. Check stop_hook_active first.", "timeout": 60}]}]
        }
      }
      ```
      Use when: Requires reading files or running commands to verify

      **Type selection guide:** command (deterministic) → prompt (judgment) → http (external) → agent (file inspection)
      **CRITICAL for Stop hooks:** ALL 4 types must handle stop_hook_active=true to prevent infinite loops.
...
anti_patterns:
  never_do:
    ...
    - "Hardcode API keys in committed settings files or `.mcp.json`"
  always_do:
    - "Audit managed, CLI, local, shared, user, and MCP surfaces before making permission changes"
    - "Use paths: frontmatter for conditional rule loading"
    - "Test permission rules against the full runtime pipeline, not just list order"
    - "Keep CLAUDE.md under 200 lines; split with @imports and .claude/rules/"
    - "Version control team-owned configuration in `.claude/settings.json`, `.claude/rules/`, and `.mcp.json` as appropriate"
...
- `*rebuild-validator` - Reconstruir o validador da squad contra o contrato real de runtime
...
HIGHEST PRECEDENCE
  |
  |  1. Managed Settings / Managed MCP Policy (cannot be overridden)
  ...
  |
  |  Project MCP Registry (.mcp.json)
  |     - Preferred mutable surface for project-owned manual MCP servers
  |     - Still subordinate to enterprise managed-mcp.json policy
  |
LOWEST PRECEDENCE
```

Array settings MERGE across scopes (concatenated, deduplicated).
MCP note: prefer `.mcp.json` for project-owned manual servers, treat `managed-mcp.json` as exclusive when present, and remember that manual definitions suppress duplicate plugin servers.
Permission note: runtime resolution is broader than `deny/ask/allow` lists and passes through safety, headless, bypass, policy, classifier, and prompt stages.
...
Runtime pipeline: safety -> headless -> bypass -> denylist -> allowlist -> classifier (`auto`, when enabled) -> prompt.
`dontAsk` converts the final prompt stage into deny. In remote sessions, default modes are constrained to `default`, `acceptEdits`, or `plan`.
...
- Treat `MEMORY.md` as a compact index, not a dump: target <= 200 lines and <= 25 KB
- Exclude derivable project state from `MEMORY.md` (code patterns, architecture, git history)
- In assistant/KAIROS flows, append new memory items to dated daily logs; a separate distillation step updates `MEMORY.md`
- Monitor `context_window.used_percentage` in status line
...
7. **Rebuild validator** -> `*rebuild-validator` aligns setup audits with the live runtime contract
...
- Writing project credentials directly into `.mcp.json` instead of env/managed secret surfaces
- Assuming custom keybindings are broadly available; external users may still be locked to defaults
- Storing derivable project state in `MEMORY.md` instead of keeping it as a concise index
...
- Ignoring managed-settings.json / managed-mcp.json for enterprise deployments
