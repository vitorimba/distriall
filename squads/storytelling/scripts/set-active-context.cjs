...
function parseArgs(argv) {
  ...
  for (const raw of argv) {
    ...
    if (raw.startsWith('--narrative-type=')) {
      args.narrativeType = raw.slice('--narrative-type='.length).trim() || null;
      continue;
    }
...
function main() {
  ...
  if (['blocked', 'not_applicable'].includes(readiness.status)) {
    fail(`Cannot activate storytelling context for ${args.business}/${args.product}: ${readiness.reason}`);
  }
  ...
  const payload = {
    active_context: {
      ...
      narrative_type: args.narrativeType,
