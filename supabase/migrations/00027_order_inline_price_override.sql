-- Migration: 00027_order_inline_price_override
-- Fix: accept optional unit_price from front-end in p_items JSONB
-- so inline price edits during order creation/update are persisted.
-- When unit_price is provided in the item JSON, it takes highest priority.
-- Fallback chain: item.unit_price > client_prices > account_prices > product_variants.sell_price

-- 1. Recreate create_order with inline price support
...
BEGIN
  ...
  LOOP
    ...
    -- Get inline price from front-end (if provided)
    v_inline_price := (v_item->>'unit_price')::NUMERIC(10,2);
    ...
    -- Price resolution: inline (front-end) > client > account > global
    v_unit_price := COALESCE(v_inline_price, v_custom_price, v_account_price, v_variant.sell_price);
...
-- 2. Recreate update_order with inline price support
